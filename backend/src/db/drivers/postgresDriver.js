/**
 * POSTGRES DRIVER — TEMPLATE, NOT YET ACTIVE.
 *
 * To activate:
 *   1. npm install pg
 *   2. Set DB_DRIVER=postgres and DATABASE_URL=postgres://user:pass@host:5432/dbname in .env
 ...
 *      JSON-shaped docs this app already produces: give every table a `data JSONB`
 *      column (plus an indexed `id TEXT PRIMARY KEY` and `org_id TEXT` for fast
 *      tenant filtering), and store/retrieve the whole doc as JSON. That's the
 *      fastest path to a working swap. A stricter column-per-field schema (matching
 *      PRISMA_SCHEMA.prisma exactly) is a follow-up optimization, not a blocker.
...
function notImplemented(name) {
  return async () => {
    throw new Error(
      `Postgres driver method "${name}" is not implemented yet. ` +
      `See src/db/drivers/postgresDriver.js for setup steps.`
    );
  };
}
...
/*
Example implementation shape once you've npm installed `pg`:
...
async function list(collection, filter = {}) {
  ...
  const where = keys.length
    ? 'WHERE ' + keys.map((k, i) => `data->>'${k}' = $${i + 1}`).join(' AND ')
    : '';
...
async function insert(collection, doc) {
  await pool.query(`INSERT INTO ${collection} (id, org_id, data) VALUES ($1, $2, $3)`, [doc.id, doc.orgId || null, doc]);
  return doc;
}
